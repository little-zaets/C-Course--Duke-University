                            ...
                            size_t match_idx)
{
  ...
  for (i = 0; i < hand->n_cards - 1; ++i)
  {
    ...
    if (count > 1 && (match_counts[i] == count || match_counts[i] == count + 1))
    {
      return i;
    }
  }
...
int is_straight_at(deck_t *hand, size_t index, suit_t fs)
{
  ...
  {
    count = is_ace_low_straight(hand, index, fs);
    if (5 == count)
    {
      return -1;
    }
                                  ...
                                  size_t idx)
{
  ...
  if (0 != n)
  {
    for (i = idx; i < idx + n; ++i, ++j)
    {
      ans.cards[j] = hand->cards[i];
    }

    for (i = 0; j < sizeof(ans.cards) / sizeof(card_t *) && i < hand->n_cards; ++i)
    {
