...
board_t *makeBoard(int w, int h, int numMines)
{
	...
	memset(b->board, 0, sizeof(int **) * b->height);
	for (i = 0; i < b->height; ++i)
	{
		b->board[i] = malloc(sizeof(int *) * b->width);
		if ( NULL == b->board[i] )
		{
			fprintf(stderr, "Malloc failed");
			freeBoard(b);
...
int countMines(board_t *b, int x, int y)
{
	...
	for (i = -1; i <= 1; ++i)
	{
		for (j = -1; j <= 1; ++j)
		{
			...
			if ( !(nx == x && ny == y) )
			{
				if (0 <= nx && ny >= 0 && nx < b->width && b->height > ny)
				{
